...
<style>
  * {{ margin: 0; padding: 0; box-sizing: border-box; }}
  ...
  #inferTable th.sort-desc::after {{ content: " ▼"; }}
  #inferTable tr.isep td {{ border-top: 2px solid #334155; }}
  .fmt-pt  {{ background: #1c1917; color: #a8a29e; border: 1px solid #292524; }}
  .fmt-trt {{ background: #431407; color: #f97316; }}
  .ap-s  {{ color: #fb923c; }}
  .ap-p  {{ color: #34d399; }}
  .ap-sb {{ color: #f472b6; }}
  .ap-pb {{ color: #2dd4bf; }}
...
<div class="tab-content" id="tab-inference">
  <div class="card">
    ...
    <p style="color:#64748b;margin-bottom:14px;font-size:0.8rem">
      FPS colour: <span style="color:#4ade80">&#9632;</span>&nbsp;&ge;100&ensp;
      <span style="color:#86efac">&#9632;</span>&nbsp;&ge;30&ensp;
    ...
    <div class="table-wrap">
      <table id="inferTable">
        <thead><tr>
          <th onclick="sortInfer(0)">Device</th>
          <th onclick="sortInfer(1)">Experiment</th>
          <th onclick="sortInfer(2)">Arch</th>
          <th onclick="sortInfer(3)">Task</th>
          <th onclick="sortInfer(4)">Size</th>
          <th onclick="sortInfer(5)">Approach</th>
          <th onclick="sortInfer(6)">Format</th>
          <th onclick="sortInfer(7)">Prec</th>
          <th onclick="sortInfer(8)">ImgSz</th>
          <th onclick="sortInfer(9)">Batch</th>
          <th onclick="sortInfer(10)">FPS</th>
          <th onclick="sortInfer(11)">Inf&nbsp;ms</th>
          <th onclick="sortInfer(12)">Total&nbsp;ms</th>
          <th onclick="sortInfer(13)">mAP50</th>
          <th onclick="sortInfer(14)">mAP50&#8209;95</th>
          <th onclick="sortInfer(15)">Model&nbsp;MB</th>
          <th onclick="sortInfer(16)">GPU&nbsp;MB</th>
        </tr></thead>
...
// ---- INFERENCE ----
const SIZE_ORDER_I = {{'nano':0,'small':1,'medium':2,'large':3}};
const EXP_ORDER_I  = {{'core_comparison':0,'class_imbalance':1,'detection_vs_segmentation':2,'input_size':3,'batch_throughput':4}};
const DEV_ORDER_I  = {{'rtx5090':0,'jetson_agx':1}};

const APPROACH_SHORT_I = {{
  'scratch':'scratch', 'pretrained':'pretrained',
  'scratch_balanced':'scratch+bal', 'pretrained_balanced':'pretrained+bal'
}};
const APPROACH_CSS_I = {{
  'scratch':'ap-s','pretrained':'ap-p',
  'scratch_balanced':'ap-sb','pretrained_balanced':'ap-pb'
}};

function fpsCss(fps) {{
  if (!fps) return '';
  if (fps >= 100) return 'color:#4ade80;font-weight:700';
  if (fps >= 30)  return 'color:#86efac';
  if (fps >= 10)  return 'color:#fbbf24';
  return 'color:#f87171';
}}

function buildInferRows(data) {{
  const sorted = [...data].sort((a,b) => {{
    const dA=DEV_ORDER_I[a.device]??9, dB=DEV_ORDER_I[b.device]??9;
    if (dA!==dB) return dA-dB;
    const eA=EXP_ORDER_I[a.experiment]??9, eB=EXP_ORDER_I[b.experiment]??9;
    if (eA!==eB) return eA-eB;
    if (a.arch!==b.arch) return a.arch.localeCompare(b.arch);
    if (a.task!==b.task) return a.task.localeCompare(b.task);
    const sA=SIZE_ORDER_I[a.size]??9, sB=SIZE_ORDER_I[b.size]??9;
    ...
    return (a.format_precision||'').localeCompare(b.format_precision||'');
  }});

  let prev = null;
  return sorted.map(r => {{
    const sep = prev && (r.experiment !== prev.experiment || r.device !== prev.device);
    prev = r;
    const apCss = APPROACH_CSS_I[r.approach] || '';
    const apLabel = APPROACH_SHORT_I[r.approach] || r.approach;
    const fmtCls = r.format === 'tensorrt' ? 'fmt-trt' : 'fmt-pt';
    const fps = r.fps;
    const expLabel = (r.experiment||'').replace(/_/g,' ');
    return `<tr${{sep?' class="isep"':''}}>
      <td><span class="device-badge device-${{r.device}}">${{r.device}}</span></td>
      <td style="color:#94a3b8">${{expLabel}}</td>
      <td><span class="arch-badge arch-${{r.arch}}">${{r.arch.toUpperCase()}}</span></td>
      <td>${{r.task||'-'}}</td>
      <td><span class="size-badge">${{r.size}}</span></td>
      <td><span class="${{apCss}}">${{apLabel}}</span></td>
      <td><span class="badge ${{fmtCls}}">${{r.format||'-'}}</span></td>
      <td>${{r.format_precision||'-'}}</td>
      <td>${{r.imgsz||'-'}}</td>
      <td>${{r.batch||'-'}}</td>
      <td style="${{fpsCss(fps)}}">${{fps!=null?fps.toFixed(1):'—'}}</td>
      <td>${{r.inference_ms!=null?r.inference_ms.toFixed(2):'—'}}</td>
      <td>${{r.total_ms!=null?r.total_ms.toFixed(2):'—'}}</td>
      <td>${{r.map50!=null?(r.map50*100).toFixed(2)+'%':'—'}}</td>
      <td>${{r.map50_95!=null?(r.map50_95*100).toFixed(2)+'%':'—'}}</td>
      <td>${{r.model_size_mb!=null?r.model_size_mb.toFixed(1):'—'}}</td>
      <td>${{r.gpu_mem_mb!=null?Math.round(r.gpu_mem_mb):'—'}}</td>
    </tr>`;
  }}).join('');
}}
...
function filterInfer() {{
  ...
  document.getElementById('inferCount').textContent = visible + ' rows';
}}

function sortInfer(col) {{
  const ths = document.querySelectorAll('#inferTable th');
  ths.forEach(t => t.classList.remove('sort-asc','sort-desc'));
  if (inferSortCol === col) {{ inferSortDir = -inferSortDir; }} else {{ inferSortCol = col; inferSortDir = 1; }}
  ths[col].classList.add(inferSortDir === 1 ? 'sort-asc' : 'sort-desc');
  const tbody = document.getElementById('inferTableBody');
  ...
  rows.sort((a,b) => {{
    ...
    if (!isNaN(an) && !isNaN(bn)) return (an-bn)*inferSortDir;
    return av.localeCompare(bv)*inferSortDir;
  }});
  rows.forEach(r => tbody.appendChild(r));
  // recount
  const vis = rows.filter(r=>r.style.display!=='none').length;
  document.getElementById('inferCount').textContent = vis + ' rows';
}}
